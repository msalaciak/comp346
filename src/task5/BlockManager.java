...
//		try {
//			out = new PrintStream(new FileOutputStream("output5.txt", true), true);
...
//		System.setOut(out);
	...
	static class AcquireBlock extends BaseThread
	{
		...
		public void run()
		{
			...
			phase1();
			//phase1 counter so we know when all the threads are finished

			phase1Counter++;

			//since they are 10 threads, this counter is compared to the threads, if it is 10, it knows all phase 1 threads are done
			...
			phase2();
			//phase2 counter so we know when all the threads are finished

			phase2Counter++;

			//since they are 10 threads, this counter is compared to the threads, if it is 10, it knows all phase 2 threads are done

			if(phase2Counter >=10) {
				System.out.println("--------------------------All threads finished phase 2-----------------------------");
			}
			s2.V();
